...
- **平面圖模式**（預設）：45 張桌子依照座位圖的相對位置擺放，圓桌畫圓、方桌畫方，大小依人數縮放；桌上小圓點代表每個位子的狀態（灰＝已有人、橘＝本次已選）。點一張桌子就在下方展開該桌位子，可跨桌連續選再一次送出
- **清單模式**：依區域分組的卡片列表，適合直接找特定桌號
...
lib/
  ...
  lunch-tables.ts                  # 午餐桌號、每桌人數與平面圖座標
  plate.ts                         # 車號正規化與格式檢查
public/
